...
import type { ApiResponse, AtelierShape, Point } from '../types';
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex gap-4">
                <button title="Rectangle" onClick={() => setCurrentTool('rect')} className={`p-3 rounded-lg ${currentTool === 'rect' ? 'bg-blue-600' : 'bg-gray-700'}`}>
                   <svg width="24" height="24" viewBox="0 0 24 24" fill="white"><path d="M3 3h18v18H3z"/></svg>
                </button>
                 <button title="Cercle" onClick={() => setCurrentTool('circle')} className={`p-3 rounded-lg ${currentTool === 'circle' ? 'bg-blue-600' : 'bg-gray-700'}`}>
                ...
                <button title="Pinceau" onClick={() => setCurrentTool('brush')} className={`p-3 rounded-lg ${currentTool === 'brush' ? 'bg-blue-600' : 'bg-gray-700'}`}>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="white"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34a.9959.9959 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>
                </button>
              </div>
            </div>
             {currentTool === 'brush' && (
                <div>
                    <h3 className="text-lg font-semibold mb-3">Épaisseur du pinceau</h3>
                    <div className="flex gap-4 items-center">
                        <button onClick={() => setBrushSize(BRUSH_SIZES.fin)} className={`px-4 py-2 rounded-lg ${brushSize === BRUSH_SIZES.fin ? 'bg-blue-600' : 'bg-gray-700'}`}>Fin</button>
                        <button onClick={() => setBrushSize(BRUSH_SIZES.moyen)} className={`px-4 py-2 rounded-lg ${brushSize === BRUSH_SIZES.moyen ? 'bg-blue-600' : 'bg-gray-700'}`}>Moyen</button>
                        <button onClick={() => setBrushSize(BRUSH_SIZES.epais)} className={`px-4 py-2 rounded-lg ${brushSize === BRUSH_SIZES.epais ? 'bg-blue-600' : 'bg-gray-700'}`}>Épais</button>
                    </div>
                </div>
             )}
             <button
              onClick={() => setShapes([])}
              className="w-full px-4 py-2 bg-red-600 hover:bg-red-700 rounded-lg font-semibold transition-colors mt-4"
            >
              Tout effacer
            </button>
          </div>
        ...
        >
          ...
          {shapes.map((shape) => {
            if (shape.type === 'circle') {
              return <circle key={shape.id} cx={shape.x} cy={shape.y} r={shape.size / 2} fill={shape.color} />;
            }
            if (shape.type === 'rect') {
              return <rect key={shape.id} x={shape.x - shape.size / 2} y={shape.y - shape.size / 2} width={shape.size} height={shape.size} fill={shape.color} />;
            }
            if (shape.type === 'path') {
                return <path key={shape.id} d={pointsToPathD(shape.points)} fill="none" stroke={shape.color} strokeWidth={shape.strokeWidth} strokeLinecap="round" strokeLinejoin="round" />
            }
            return null;
          })}
        </svg>
